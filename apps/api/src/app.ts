...
export function buildApp() {
  ...
  app.setErrorHandler((error, _request, reply) => { if (error instanceof z.ZodError) return reply.code(400).send({ message: "Validation failed", issues: error.issues.map(({ path, message }) => ({ path, message })) }); const status = (error as Error & { statusCode?: number }).statusCode ?? 500; return reply.code(status).send({ message: status === 500 ? "Internal server error" : error.message }); });
  return app;
}
